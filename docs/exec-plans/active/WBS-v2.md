...
> 프로젝트: k-stock-merchant | 작성자: 배영환 | 작성일: 2026-05-13 | 최종 업데이트: 2026-05-17 (1.13 뉴스 실제이벤트 교체 — 국제뉴스 50개·기업뉴스 157개 실제 2025~2026 한국 금융시장 이벤트 기반 전면 교체 + 국제뉴스 1000자 이상 보장 + PC 브라우저 호환성 패치. 이전: 1.12 뉴스 시스템 고도화 PR #54 / 1.11 UI 폴리시)
...
| 1.10.8 | [Front] ★★★ 거래소 모멘텀 펄스 ↔ OBV 위치 스왑 — `StockChart.jsx`의 세로 스택 순서를 (메인 → MACD → OBV)에서 (메인 → OBV → MACD)로 변경. 패널 렌더 순서 + Y좌표 계산 + 도움말 오버레이 섹션 순서 동시 조정 | 송원호 | 대기 | 0% | 05/17 | 05/17 | 0.2 | src/components/game/StockChart.jsx, src/pages/MarketPage.jsx | P1 |
| 1.10.9 | [Front] ★★ 거래소 합계/금액 텍스트 좌측 정렬 — `BulkBuyPanel` 하단 요약 라인의 `합계` 박스(현재 `text-right` + 우측 정렬) / `BulkSellPanel` 매도 수익 박스를 좌측으로 이동. flex `justify-between` 좌우 swap 또는 두 영역 순서 교체 | 송원호 | 대기 | 0% | 05/18 | 05/18 | 0.2 | src/pages/MarketPage.jsx | P2 |
