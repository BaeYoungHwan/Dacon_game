...
| 이름 | 역할 | 담당 |
|------|------|------|
| 배영환 | 시니어 + Claude Code | 아키텍처, 게임 로직, store, 통합, 배포, 종목 선정, 가격 데이터 하드코딩, 화면 뼈대 구조 |
| 송원호 | 초급 React + Claude Code | 컴포넌트 디자인, 페이지 스타일링, 상점 모달, 기업·국제 뉴스 콘텐츠 정리 |
...
| 이름 | 수정 가능 영역 |
|------|---------------|
| 배영환(Claude) | `src/store/`, `src/lib/`, `src/App.jsx`, `src/data/stockData.json`, Supabase 관련, 화면 뼈대 구조(마크업·데이터 바인딩) |
| 송원호(Claude) | `src/pages/`(스타일링), `src/components/`(스타일링), `src/data/news-events.json` |
