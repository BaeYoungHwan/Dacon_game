...
매주 뉴스 이벤트가 터지고 코스피와 환율이 흔들린다. 21개 종목 중 랜덤으로 배정된 10개 종목을 분석하고, 거래소·정보상·기술상을 오가며 최적의 투자 타이밍을 잡아라.
...
| **GamePage** | 메인 허브 — 포트폴리오 현황, 뉴스 패널, 코스피 차트, 턴 진행 |
| **MarketPage** | 한국거래소 — 종목 분석 홀로그램 + 매수/매도 키오스크 |
| **InfoMerchantPage** | 정보상 — 국제 뉴스 / 기업 뉴스 / 추천 종목 |
| **TechMerchantPage** | 기술상 — 차트 지표 구매(MA·볼린저밴드·MACD·OBV) + 깜짝 종목 해금 (10턴 이후) |
| **ResultPage** | 최종 자산 평가(등급), 랭킹 등록/조회 |
...
│   │   ├── KospiChart.jsx      # 코스피 지수 차트
│   │   ├── NewsPanel.jsx       # 뉴스 이벤트 출력
│   │   ├── Portfolio.jsx       # 보유 주식 + 현금 잔액
│   │   ├── SettingsModal.jsx   # 설정 모달 (음량 등)
...
│   └── news-events.json        # 날짜별 뉴스 이벤트
├── lib/
│   ├── audioManager.js         # 오디오 재생 유틸
...
3. 뉴스 — news-events.json에서 해당 날짜 이벤트 매칭
   └─ 기업뉴스(sector ≠ '전체') / 국제뉴스(sector = '전체') 분리 표시
